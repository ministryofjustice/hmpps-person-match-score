...
from .utils import standardisation_functions
...
def match():
    try:
        ...
        response = score(data)
        event_logger(__name__).info(
            "PiCPersonMatchScoreGenerated",
            extra={"custom_dimensions": custom_dimensions_from(response)},
        )
        return response
    except Exception as e:
...
def score(data):
    ...
    row_arrow_1 = pa.Table.from_pandas(row_1, schema=cleaned_data_schema, preserve_index=False)
    row_arrow_2 = pa.Table.from_pandas(row_2, schema=cleaned_data_schema, preserve_index=False)

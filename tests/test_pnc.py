...
def test_pnc_1(spark_session):
    ...
    df = spark_session.createDataFrame(Row(**x) for x in pnc_list)
    df = standardise_pnc_number(df, "pnc_number")
...
def test_pnc_2(spark_session):
    ...
    df = spark_session.createDataFrame(Row(**x) for x in pnc_list)
    df = standardise_pnc_number(df, "pnc_number", drop_orig=False)
